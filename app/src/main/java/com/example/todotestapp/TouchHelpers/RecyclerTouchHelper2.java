...
public class RecyclerTouchHelper2 extends ItemTouchHelper.SimpleCallback {

    private ToDoAdapter2 adapter;
    private Weakness activity;

    public RecyclerTouchHelper2(ToDoAdapter2 adapter, Weakness activity){
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dx, float dy, int actionstate, boolean currentlyActive){
        ...
        if(dx>0){
            ...
            bg = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.redswipe));
        }
        else{
