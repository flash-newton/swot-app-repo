...
public class RecyclerTouchHelper3 extends ItemTouchHelper.SimpleCallback {

    private ToDoAdapter3 adapter;
    private Opportunity activity;

    public RecyclerTouchHelper3(ToDoAdapter3 adapter, Opportunity activity){
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dx, float dy, int actionstate, boolean currentlyActive){
        ...
        if(dx>0){
            ...
            bg = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.opporswipe));
        }
        else{
